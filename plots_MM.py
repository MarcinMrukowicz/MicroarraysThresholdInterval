...
def split_into_subsets(S, n):
    q, r = divmod(len(S), n)
    print(f"q = {q} r = {r}")
    subsets = [S[i * q + min(i, r):(i + 1) * q + min(i + 1, r)] for i in range(n)]
    return subsets

print([len(x)for x in np.array_split(np.arange(1200),12)])
print([len(x)for x in split_into_subsets(np.arange(1000),12)])
